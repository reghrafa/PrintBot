/*
 * USBasp_UART_Terminal.c
 *
 * Created: 06.10.2014 22:29:56
 *  Author: Fabian
 */ 
// Project:		UART device-side firmware for USBASP
// Author:		Konrad Morelowski <92linux92@gmail.com>
// License:		GNU GPL v2 or later
// Date:		2012-09-07
// Previous author as below:
...
uchar usbFunctionSetup(uchar data[8]) 
{
	...
	else if (data[1] == USBASP_FUNC_SET_UART1) 
	{
		UCSRA = data[2];
		UCSRB = data[3];
		UCSRC = data[4];
	...
	else if (data[1] == USBASP_FUNC_SET_UART2) 
	{
		UBRRH = data[2];
	} 
	else if (data[1] == USBASP_FUNC_TXRX_UART) 
	{
		if( UCSRA & (1<<RXC) ) 
		{
			replyBuffer[0] = UDR;
			replyBuffer[1] = UCSRA; // for frame error detection etc.
			len = 2;
		}
		if( data[2] == 1 ) 
		{
...
int main(void) 
{
	...
	PORTD = 0;
	PORTB = 0;
	/* all outputs except PD2 = INT0 and PD0 = RXD*/
	DDRD = ~(1<<PD2) & ~(1<<PD0);
